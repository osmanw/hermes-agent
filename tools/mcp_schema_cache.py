...
def get_cached_entry(server_name: str, fingerprint: str) -> Optional[dict]:
    ...
    # ``ttlMs: 0`` is "no caching hint" (SEP-2549), not "expires immediately". Treating it as an
    # instant expiry made every ``lazy: true`` server a permanent cache miss, so lazy silently
    # degraded to an eager connect on every startup (observed with context7).
